...
def dfs_paths(graph, start, goal):
    ...
    з одного міста в інше. Варіант відвідування
    одного і того же міста більше одного разу,
    не розглядається.
    """
    stack = [(start, [start])]
    while stack:
...
def get_routes(request, form) -> dict:
    ...
    if not len(all_ways):
        raise ValueError('Маршрута, задовільняющого умови не існує')
    if cities:
    ...
    for route in right_ways:
        ...
        if total_time <= travelling_time:
            routes.append(tmp)
    if not routes:
        raise ValueError('Час в дорозі більше заданого')
    sorted_routes = []
    if len(routes) == 1:
